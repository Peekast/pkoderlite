...
#include <libpkst/pkst_iocontext.h>
#include <libpkst/pkst_audio.h>
#include <libpkst/pkst_routine.h>
...
type EncoderConfig struct {
	...
	Outputs [MAX_ENCODER_OUTPUTS]*EncoderOutput // The configurations for outputs. A maximum of four outputs are supported.
}
...
// makeEncoderConfig creates an EncoderConfig based on the provided parameters.
// It configures the input, audio settings, and output destinations for the encoder.
func makeEncoderConfig(protocol string, listenURI string, timeout int, rtmpDst []string, mpegts []string) *EncoderConfig {
	//func makeEncoderConfig(id string, segmentOutput string, listenOn *ListenData, timeout int, rtmpDst []string) *EncoderConfig {
...
type Encoder struct {
	...
	server     *LoopbackListener    // The TCP server for receiving connections.
...
func (e *Encoder) GetDestination(n int) (string, bool) {
	if n < int(e.config.outs_len) {
		return C.GoString(e.config.outs[n].dst), true
	}
	return "", false
}
...
// error.
func NewEncoder(protocol string, listenURI string, timeout int, rtmpDst []string, mpegts []string) (*Encoder, error) {
	config := makeEncoderConfig(protocol, listenURI, timeout, rtmpDst, mpegts)
...
func allocEncoderConfig(in string, listen bool, timeout int, inType string) (*Encoder, error) {
	...
	cTcpstats := C.CString(server.URI())
...
func (pe *Encoder) addAudioEncoderConfig(codec string, bitrateBps, channels, sampleRate int) error {
	...
	res := C.pkst_add_audio_encoder_config(pe.config, &audioConfig, C.int(0))
	if res != 0 {
...
func (pe *Encoder) Start() error {
	...
	doOnce := func() {
		C.pkst_dump_encoder_config(pe.config)

		ret := C.pkst_start_encoder_routine(pe.config, &pe.routine, nil, nil)
		if ret != 0 {
			pe.cleanup()
			pe.server.Close()
		...
		go func() {
			...
			defer socket.Close()
			for {
				mType, _, err := ReadMessage(socket)
				if err != nil {
					goto end
				}
				switch mType {
